<?php
	...
	class OemproConnect {
		...
		private function sendRequest($Command, array $arrParams) {
			...
			if ($this->bolLogged) $arrParams['SessionID'] = $this->SessionID;
		...
		public function userLogin($User, $Password) {
			...
			if ($arrResponse['Success'] === false) {
				...
				$this->throwErrors($errorCode, $errors);
			} else {
		...
		public function sendCampaign($CampaignID) {
			...
			$arrParams = array();
			$arrParams['CampaignStatus'] = 'Ready';
			$arrParams['ScheduleType'] = 'Immediate';
			$arrParams['CampaignID'] = $CampaignID;
		 ...
		 */
		public function getLists($OrderField = "Name", $OrderType = "ASC", $RecordsFrom = 0, $RecordsPerRequest = 10) {
			$this->checkLogin();
		 ...
		 */
		public function setupEmail($EmailID, $Name, $FromEmail, $FromName, $HTMLContent,
									$PlainContent, $ReplyToEmail, $ReplyToName, $Subject) {

			$this->checkLogin();

			if (is_null($EmailID))
				$EmailID = $this->createEmail($Name);

			$arrParams = array();
			$arrParams['EmailID'] = $EmailID;
			$arrParams['EmailName'] = $Name;
			$arrParams['FromEmail'] = $FromEmail;
			$arrParams['FromName'] = $FromName;
			$arrParams['HTMLContent'] = $HTMLContent;
			$arrParams['ImageEmbedding'] = 'Disabled';
			$arrParams['Mode'] = 'Empty'; //envia o html
			$arrParams['PlainContent'] = 'Plain email body';
			$arrParams['RelTemplateID'] = '';
			$arrParams['ReplyToEmail'] = $ReplyToEmail;
			$arrParams['ReplyToName'] = $ReplyToName;
